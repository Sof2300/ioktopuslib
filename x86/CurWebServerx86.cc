...
#include "webserver/socket/src/Socket.h"
...
int CurWebServerx86::headers(){return lastreq->req->allheaders.size();}
std::string CurWebServerx86::header(int i){
	size_t pos=0;
	while(pos!=std::string::npos && i>0){
		pos=lastreq->req->allheaders.find("\n",pos+1);
		if(pos!=std::string::npos) i--;
	}
	if(i==0) {
		size_t pos2=lastreq->req->allheaders.find("\n",pos+1);
		size_t pos3=lastreq->req->allheaders.find(":",pos+1);
		if(pos2==std::string::npos) pos2=lastreq->req->allheaders.size();
		if(pos3==std::string::npos) return std::string("");
		return lastreq->req->allheaders.substr(pos3+1,pos2-pos3-1);
	}
	return std::string("");
...
std::string CurWebServerx86::headerName(int i){
	size_t pos=0;
	while(pos!=std::string::npos && i>0){
		pos=lastreq->req->allheaders.find("\n",pos+1);
		if(pos!=std::string::npos) i--;
	...
	if(i==0) {
		size_t pos3=lastreq->req->allheaders.find(":",pos+1);
		if(pos3==std::string::npos) return std::string("");
		return lastreq->req->allheaders.substr(pos+1,pos3-pos-1);
	}
	return std::string("");
