...
#ifdef ESP8266BUILD
// Ensure Arduino types (String, __FlashStringHelper, F()) are available
#include <Arduino.h>
// Create a unique identifier for each FFX expansion using the current __LINE__
// Use a plain static const char array (no custom section attribute). Placing
// objects into a custom section caused section type conflicts across TUs on
// some toolchains. Removing the section attribute keeps the intent (unique
// flash/string storage per expansion) while avoiding linker conflicts.
#define __GS_CONCAT(a,b) a##b
#define __GS_CONCAT2(a,b) __GS_CONCAT(a,b)
#define FFX(string_literal) (__extension__({ static const char __GS_CONCAT2(__genstr_,__LINE__)[] = (string_literal); &__GS_CONCAT2(__genstr_,__LINE__)[0]; }))
#define RF(x) (FFX(x))
#else
#define RF(x) x
#endif
...
//	Serial.println(String()+"DataManager::getToken1:  i "+i+" j "+j );
	...
	if(index==0 && i>-1) {
	  	if(res) {
	  		*res=pile.substring(i+1,pile.length());
//	  		Serial.println(String()+"DataManager::getToken1.5: '"+*res+"'");
	  	}
		String ret=pile.substring(0,i);
//		Serial.println(String()+"DataManager::getToken1.5:  ret '"+ret+"'");
		return ret;
	}
...
// arduino String class replace function is failing for some reason
	while (src.indexOf(pattern)>-1)	src.replace(pattern, newval);
	return src;
/*
	int start=src.indexOf(pattern);
	while (start>-1)	{
		Serial.println(String()+"loadFileToString :"+src.indexOf(pattern));
//		filecontent=filecontent.replace(pattern, String("http:\/\/"));
		start=src.indexOf(pattern);
		int end=start+pattern.length();
		filecontent=src.substring(0,start) + replacement+src.substring(end, src.length());
	}
*/
...
bool isBinary (String str) { // 0 or 1
...
bool isInteger(String str) {
	...
	if(String(str.toInt())!=str) return false;
	return true;
}
...
bool getPathIsAbsLink(GenString val){	// n.b.: path starting directly with key is not considered a link, e.g. a/b/c/
	if(val.empty()) return false;
	if(val[0]=='/') return true;	// start with "/"	: absolute link
	return false;// could do better, e.g. check syntax errors, etc
}
bool isAbsLink(String val){return getPathIsAbsLink(val.c_str());}
...
bool isLink(String val){return getPathIsLink(val.c_str());}
...
/*	// too many compilation problems because of the same name function in std
...
*/
static std::string tostring(uint64_t num){
	uint8_t i = 0;  uint64_t n = num;
	char str[24];//should be 21
	do i++;
	while ( n /= 10 );
