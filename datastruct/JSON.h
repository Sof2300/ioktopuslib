...
// can't parse properly numbers without "" quotes around, fail on third entry
...
inline void printErrorJSON(std::string symbol, std::string &json, unsigned int indexp, std::string *err, std::string mess=""){
	...
	*err+=mess+"'"+symbol+"' expected at line :"+ tostring(countLines(json,indexp))+" :"+printExtract(json,indexp);
};
