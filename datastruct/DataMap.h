...
bool isFloat(String tString) {
	String tBuf;
	boolean decPt = false;
	if(tString.charAt(0) == '+' || tString.charAt(0) == '-') tBuf = tString.substring(1); else tBuf = tString;
...
//	void update(String key, float fvalue){update(key,String(fvalue));}
	...
	String getJson(std::vector<String> hidden={}, bool withouter=true, bool hidefield=false){
		String json;
		...
		for(auto it=data.begin();it!=data.end();it++){
			bool found=false;
			for(String s : hidden) {
	//			Serial.println(String()+"getJson:"+s+"=="+it->first);
				if(it->first==s) {found=true;break;}
			}
	//		Serial.println(String()+"getJson: found"+found);
			if(hidefield && found) continue;	// hide field
			if(!firstentry) json+=","; else firstentry=false;
			json+=String()+"\""+it->first+"\":";
			bool subarray=false;
			String val=it->second;
			if(val[0]=='[') subarray=true;
			if(found || it->second.length()==0) json+="\"\"";		// hide the value not the field
			//else if(isFloat(it->second)) json+=it->second; // can't read numbers without quotes yet json+=it->second;
			else if(subarray) json+=it->second;
