#ifndef SIMPLEFSESP8266_H
#define SIMPLEFSESP8266_H
...
class SimpleFSESP8266 {
public:
	SimpleFSESP8266(){ CurFS.begin();};
	// explicit begin to match API used by sketches
	void begin(){ CurFS.begin(); };

	// Storage info methods (used by DataManager)
	...
	void printInfo(){
		...
		Serial.println(String()+"- used space : "+us+" byte ("+String(peru/100.0)+"%)");
		Serial.println(String()+"- available space : "+as+" byte ("+String(perf/100.0)+"%)");
	...
	// convenience overload to accept Arduino String directly (used by sketch)
	unsigned int rewriteFile(const char *path, String content){
		return rewriteFile(path, content.c_str(), content.length());
	}

	unsigned int rewriteFile(String path, String content){
		return rewriteFile(path.c_str(), content.c_str(), content.length());
	}
