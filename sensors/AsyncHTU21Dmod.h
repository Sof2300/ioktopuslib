...
//	 Serial.println(String()+"HTU21D::readHumidityAsync maxmillis:"+String(maxmillis)+", humts:"+String(humts)+", millis:"+millis());
	...
	if((tempStarted || humStarted) && (millis()-checkts)<DELAY_INTERVAL) return false; // we should wait more between checks
//	Serial.println("HTU21D::readTemperatureAsync1");
	if(tempStarted) {readTemperatureAsync(temperature,0);return false;} //busy
//	Serial.println("HTU21D::readTemperatureAsync2");
	if(!humStarted) {startReadValue(TRIGGER_HUMD_MEASURE_NOHOLD);humStarted=true;return false;}
//	Serial.println("HTU21D::readTemperatureAsync3");
	if((millis()-readts)>MAX_WAIT) {humStarted=false;readTimeout=true;}//return false;}//timeout
//	Serial.println("HTU21D::readHumidityAsync checking value");
	checkValue();

	if(valueReady) {
//		Serial.println("HTU21D::readHumidityAsync value ready");
		uint16_t rawHumidity = readResultValue();;
//		Serial.println(String()+"HTU21D::readHumidityAsync rawHumidity "+rawHumidity+" "+(rawHumidity&0x03));//status bits ?  https://github.com/Porokhnya/GreenhouseProject/blob/master/UniversalSensorsModule/HTU21D.cpp#L253
...
#define HTU21D_ON 0x04                       //heater ON
#define HTU21D_OFF 0xFB                      //heater OFF
void HTU21D::setHeater(bool on=false)
{
	uint8_t userRegisterData = readUserRegister();


  if(on) userRegisterData |= HTU21D_ON;
  else userRegisterData &= HTU21D_OFF;

  writeUserRegister(userRegisterData);
}
