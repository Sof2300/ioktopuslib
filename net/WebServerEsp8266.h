...
public:
	...
	WebServerEsp8266(unsigned int port0=80): port(port0), server(port0){};
	...
	void begin(String mdnsn){
		if(mdnsn.length()>0) setMdnsName(mdnsn.c_str());
#if USEMDNS
		if(!mdnsname.empty()) {
...
//	void on(std::string path, HTTPMethod getpost, void (*func)()){
	void on(std::string path, HTTPMethod getpost, THandlerFunction func){
...
//	void onNotFound(void (*func)()){server.onNotFound(func);};
	void onNotFound(THandlerFunction func){server.onNotFound(func);};
	void handleClient(){
#if USEMDNS
	...
	size_t streamFile(std::string path, std::string contentType,unsigned long start=0,unsigned long stop=0){
		if(SimpleFS.exists(path)) {
			File file = CurFS.open(path.c_str(), RF("r"));
			size_t sent =0;
			if(file){
				println(std::string()+RF("CurWebServerEsp8266::streamFile: sending ")+tostring(start)+" - "+tostring(stop)+", file size: "+tostring(file.size()));
				...
				println(std::string()+RF("CurWebServerEsp8266::streamFile: sent ")+tostring(sent));
				file.close();
			}
			return sent;
	...
	unsigned int streamFilePart(File file, std::string contentType,unsigned long start,unsigned long stop) {
		println(std::string()+RF("CurWebServerEsp8266::streamFilePart: sending ")+tostring(start)+" - "+tostring(stop));
	...
	bool uploadFile(){
		...
		println(GenString()+"handleFileUpload "+tostring(upload.status)+" "+tostring(UPLOAD_FILE_START));
