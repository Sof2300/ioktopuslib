...
public:
	...
	//	printHeaders();
		if(server.hasHeader("x-Date")) {	// resync server time from client
			...
			unsigned long nzerotime=str.toInt()-millis()/1000;
			//	Serial.println(String()+"nboottime: "+nboottime+" str.toInt():"+str.toInt());

			if( (nzerotime+5)<boottime || (nzerotime-5)>boottime) {	// will be called if out of sync: first time or millis cycled
				StringMapEvent nmap;
				nmap.insertValue("x-Date",str.c_str());
				String tz=server.header("x-TimeZone").c_str();
		//		Serial.println(String()+"BasicServer:"+tz);
				if(server.hasHeader("x-TimeZone")) nmap.insertValue("x-TimeZone",tz.c_str());	// obviously this is UTC not timezone time
				//bool b=
				emit("x-Date", &nmap);
				Serial.println(String()+"BasicServer new boottime:"+nzerotime+" old boottime:"+boottime);
				boottime=nzerotime;
			} //else Serial.println(String()+"Resync time diff: "+(boottime-nzerotime));;
		}
